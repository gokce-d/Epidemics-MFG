...
def plot_vaccination_only(t_grid, nu, nblocks, title, T):
    ...
    for k in np.arange(nblocks):
        c = next(color)
        ax.plot(t_grid, nu[k, :], label="Vaccination, block {}".format(k+1), c=c, linestyle="-")

    ax.legend(fontsize="8")
    ax.set_title("Vaccination Control (nu)")
    plt.setp(ax, xlim=(0, T))
    plt.suptitle(title)
    plt.show()