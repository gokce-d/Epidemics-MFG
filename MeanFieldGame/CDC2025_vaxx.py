...
def contact_rate_control_calc(
    ...
    if lambda_type == 0 or lambda_type == 1:
        alpha_s = np.reshape(lambda_s, (n_blocks, 1)) + np.reshape(
            beta / (2 * c_lambda), (n_blocks, 1)
        ) * Z * (u[0:n_blocks, :] - u[n_blocks : 2 * n_blocks, :])
        alpha_i = np.tile(np.reshape(lambda_i, (n_blocks, 1)), Nt)
        alpha_r = np.tile(np.reshape(lambda_r, (n_blocks, 1)), Nt)
        nu = np.reshape(kappa / (2 * c_nu), (n_blocks, 1)) * (
            u[0:n_blocks, :] - u[2 * n_blocks : 3 * n_blocks, :]
        )
    if lambda_type == 2:
        alpha_s = lambda_s + np.reshape(beta / c_lambda, (n_blocks, 1)) * Z * (
            u[0:n_blocks, :] - u[n_blocks : 2 * n_blocks, :]
        )
        alpha_i = lambda_i
        alpha_r = lambda_r
        nu = np.reshape(kappa / (2 * c_lambda), (n_blocks, 1)) * (
            u[0:n_blocks, :] - u[2 * n_blocks : 3 * n_blocks, :]
        )
...
def cdc_rate_ODE_p(
    ...
    if death == 1:
        ...
        rate_p_R = (
            rho * gamma * p[n_blocks : 2 * n_blocks]
            - kappa * p[(n_states - 1) * n_blocks : n_states * n_blocks]
        )
...
def cdc_rate_ODE_u(
    ...
    if lambda_type == 2:
        lambda_s = lambda_s(t)
    rate_u_S = (
        beta * alpha_s * Z * (u[0:n_blocks] - u[n_blocks : 2 * n_blocks])
        - c_lambda * ((lambda_s - alpha_s) ** 2)
        + kappa * nu * (u[0:n_blocks] - u[2 * n_blocks : 3 * n_blocks])
        - c_nu * ((nu) ** 2)
    )
    # ((beta**2)/(2*c_lambda)) * (Z**2) * (u[n_blocks:2*n_blocks]- u[0:n_blocks])**2)
    rate_u_I = -(
        gamma * (u[2 * n_blocks : 3 * n_blocks] - u[n_blocks : 2 * n_blocks]) + c_inf
    )
    rate = []
    for k in np.arange(n_blocks):
...
def simulateEQ_contact_rate(
    ...
    if lambda_type == 0 or lambda_type == 1:
        before_p, before_u, lambda_s, lambda_i, lambda_r = initializer(
            n_blocks,
            n_states,
            Nt,
            lambda_s_in,
        ...
        alpha_s, alpha_i, alpha_r, nu = contact_rate_control_calc(
            lambda_s,
            lambda_i,
            lambda_r,
            beta,
        ...
        after_p = cdc_solver_KFP(
            ...
            n_states,
            n_blocks,
            np.reshape(p_0, (n_blocks * n_states)),
            t_grid,
            T,
        )
        after_u = cdc_solver_HJB(
            ...
            c_nu,
            n_blocks,
            np.reshape(u_T, (n_blocks * n_states)),
            t_grid,
            T,
        )
        ...
        while (la.norm(after_p - before_p) > epsilon) or (
            la.norm(after_u - before_u) > epsilon
        ):
            Z = Z_calculator(
                n_blocks, block_dens, lambda_i, graphon, after_p, lambda_type
            )
            inter_Z = interp1d(t_grid, Z)
            alpha_s, alpha_i, alpha_r, nu = contact_rate_control_calc(
                lambda_s,
    ...
    if lambda_type == 2:
        ...
        inter_alpha_s = interp1d(t_grid, alpha_s)
        inter_nu = interp1d(t_grid, nu)  # interpolate nu here?
        after_p = cdc_solver_KFP(
            death,
            inter_alpha_s,
            inter_Z,
        ...
        while (la.norm(after_p - before_p) > epsilon) or (
            ...
            inter_alpha_s = interp1d(t_grid, alpha_s)
            inter_nu = interp1d(t_grid, nu)  # added
            before_p = after_p.copy()
            before_u = after_u.copy()
            after_p = cdc_solver_KFP(
                death,
